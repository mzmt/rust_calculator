...
/// Whether the number had a `+` or `-` sign.
 ...
 BadUrl ( CompactCowStr < 'a > ) , /// A `<bad-string-token>`
 ///
 /// This token always indicates a parse error.
 BadString ( CompactCowStr < 'a > ) , /// A `<)-token>`
 ///
...
let current = self . position ; let start = self . input [ 0 .. current ] . rfind ( | c | matches ! ( c , '\r' | '\n' | '\u{c}' ) ) . map_or ( 0 , | start | start + 1 ) ; let end = self . input [ current .. ] . find ( | c | matches ! ( c , '\r' | '\n' | '\u{c}' ) ) . map_or ( self . input . len ( ) , | end | current + end ) ; & self . input [ start .. end ]
...
1
} ; source = & source [ offset .. ] ; position += offset ; location . line += 1 ; location . column = 0 ;
...
/// The line number, starting at 0 for the first line.
 pub line : u32 , /// The column number within a line, starting at 0 for first the character of the line.
 pub column : u32 ,
...
Ok ( value ) => QuotedString ( value ) , Err ( value ) => BadString ( value )
 ...
 fn consume_quoted_string < 'a > ( tokenizer : & mut Tokenizer < 'a > , single_quote : bool ) -> Result < CompactCowStr < 'a > , CompactCowStr < 'a >> {
...
let start_pos = tokenizer . position ( ) ; while ! tokenizer . is_eof ( ) {
{
enum Case {
Case1 = 1 , Case2 = 2 , Case3 = 3 ,
